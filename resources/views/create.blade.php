...
@section('contenu')
    <section class="container">
        <form action="{{ route('eleve.store') }}" method="POST" enctype="multipart/form-data">
            ...
            {{-- Date de Naissance --}}
            <div class="mb-3">
                <label for="dateNaiss" class="form-label">Ajouter une date de naissance</label>
                <input type="date" class="form-control" @error('dateNaiss') is-invalid @enderror name="dateNaiss"
                    id="dateNaiss" value="{{ old('dateNaiss') }}">
                @error('dateNaiss')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            {{-- Photo --}}
            <div class="mb-3">
                <label for="image" class="form-label">Entrez votre date de naissance</label>
                <input type="file" class="form-control" @error('image') is-invalid @enderror name="image"
                    id="image" placeholder="Votre image" value="{{ old('image') }}">
                @error('image')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
